...
const { generarTags, generarTagsTCP, arregloTagsPS12Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS12 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS12_DATA_PROCESS'], 'PIS12')}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS12TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS12_TA_12_01','Granmar_PS12_TA_12_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS12Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(12)] = ex.Value;     
                    });
                    dataCache.PS12Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS12 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS12'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS12'){
...
module.exports = { handleSubscriptionPS12 }
