...
const { generarTags, generarTagsTCP, arregloTagsPS27Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS27 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS27_DATA_PROCESS'], 'PIS27')}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
...
const getPS27TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS27_TA_27_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPS27Antenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPS27Antenas()}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS27 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS27'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS27'){
