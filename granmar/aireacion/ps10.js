...
const { generarTags, generarTagsTCP, arregloTagsPS10Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS10 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS10_DATA_PROCESS'], 'PIS10')}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
...
const getPS10TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS10_TA_10_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPS10Antenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPS10Antenas()}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS10 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS10'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS10'){
