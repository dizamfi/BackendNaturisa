...
const { arregloTagsAntenasAllGranmar } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getAntenasAllGranmar = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsAntenasAllGranmar()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const executeAllFuntions = () => {
    getAntenasAllGranmar()
}

const handleSubscriptionAntenasAllGranmar = (ws, message) => {
    if (message.toString() === 'subscribeGRANMAntenas'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMAntenas'){
...
module.exports = { handleSubscriptionAntenasAllGranmar }
