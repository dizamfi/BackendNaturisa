...
const { generarTags, generarTagsTCP, arregloTagsPS26Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS26 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS26_DATA_PROCESS'], 'PIS26')}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
...
const getPS26TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS26_TA_26_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPS26Antenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPS26Antenas()}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS26 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS26'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS26'){
