...
const { generarTags, generarTagsTCP, arregloTagsPS02Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS02 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS02_DATA_PROCESS'], 'PIS2')}},"ClientCookie":"myReadTagRequest2"}\n`;
            let obje = {};
...
const getPS02TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS02_TA_02_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPS02Antenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPS02Antenas()}},"ClientCookie":"myReadTagRequest2"}\n`;
            let obje = {};
            client.write(tagReadCommand);
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS02Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(12)] = ex.Value;     
                    });
                    dataCache.PS02Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS02 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS02'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS02'){
...
module.exports = { handleSubscriptionPS02 }
