...
const { generarTags, generarTagsTCP, arregloTagsPS05Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS05 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS05_DATA_PROCESS'], 'PIS5')}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS05TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS05_TA_05_01','Granmar_PS05_TA_05_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS05 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS05'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS05'){
