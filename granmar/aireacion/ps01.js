...
const { generarTags, generarTagsTCP, arregloTagsPS01Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS01 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS01_DATA_PROCESS'], 'PIS1')}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS01TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS01_TA_01_01','Granmar_PS01_TA_01_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS01Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(12)] = ex.Value;     
                    });
                    dataCache.PS01Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS01 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS01'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS01'){
...
module.exports = { handleSubscriptionPS01 }
