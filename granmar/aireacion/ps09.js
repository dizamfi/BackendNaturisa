...
const { generarTags, generarTagsTCP, arregloTagsPS09Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS09 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS09_DATA_PROCESS'], 'PIS9')}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS09TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS09_TA_09_01','Granmar_PS09_TA_09_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS09Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(12)] = ex.Value;     
                    });
                    dataCache.PS09Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS09 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS09'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS09'){
...
module.exports = { handleSubscriptionPS09 }
