...
const { generarTags, generarTagsTCP, arregloTagsPS11Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS11 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS11_DATA_PROCESS'], 'PIS11')}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
...
const getPS11TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS11_TA_11_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPS11Antenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPS11Antenas()}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
            client.write(tagReadCommand);
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS11Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(12)] = ex.Value;     
                    });
                    dataCache.PS11Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS11 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS11'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS11'){
...
module.exports = { handleSubscriptionPS11 }
