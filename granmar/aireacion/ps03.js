...
const { generarTags, generarTagsTCP, arregloTagsPS03Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS03 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS03_DATA_PROCESS'], 'PIS3')}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
...
const getPS03TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS03_TA_03_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPS03Antenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPS03Antenas()}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
            client.write(tagReadCommand);
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS03Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(12)] = ex.Value;     
                    });
                    dataCache.PS03Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS03 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS03'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS03'){
...
module.exports = { handleSubscriptionPS03 }
