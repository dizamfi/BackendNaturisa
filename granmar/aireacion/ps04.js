...
const { generarTags, generarTagsTCP, arregloTagsPS04Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS04 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS04_DATA_PROCESS'], 'PIS4')}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS04TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS04_TA_04_01','Granmar_PS04_TA_04_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS04 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS04'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS04'){
