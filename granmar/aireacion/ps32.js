...
const { generarTags, generarTagsTCP, arregloTagsPS32Antenas } = require('../../../helpers/granmar/arregloTagsAireacion');
...
const getPS32 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Granmar_PS32_DATA_PROCESS'], 'PIS32')}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
...
const getPS32TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Granmar_PS32_TA_32_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPS32Antenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPS32Antenas()}},"ClientCookie":"myReadTagRequest3"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS32 = (ws, message) => {
    if (message.toString() === 'subscribeGRANMPS32'){
    ...
    } else if (message.toString() === 'unsubscribeGRANMPS32'){
