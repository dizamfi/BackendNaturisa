...
const { arregloTagsConsumisaEB, arregloTagsConsumisaEBGeneral } = require('../../../helpers/granmar/arregloTagsBombeo');
...
const getEBConsumisa = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsConsumisaEB()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    ...
                    dataCache.EBConsumisa = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(15)] = ex.Value;     
                    });
                    dataCache.EBConsumisa.push(obje)
                    obje = {}
                    completedFuntions++
...
const getEBConsumisaGral = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsConsumisaEBGeneral()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.EBConsumisaGral = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(20)] = ex.Value;     
                    });
                    dataCache.EBConsumisaGral.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionEBConsumisa = (ws, message) => {
    if (message.toString() === 'subscribeCONSUEB'){
    ...
    } else if (message.toString() === 'unsubscribeCONSUEB'){
...
module.exports = { handleSubscriptionEBConsumisa }
