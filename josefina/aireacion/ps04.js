...
const { generarTags, generarTagsTCP, arregloTagsPS02Antenas, arregloTagsPS03Antenas, arregloTagsPS04Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS04 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS04'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS04TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS04_TA_04_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS04Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(18)] = ex.Value;     
                    });
                    dataCache.PS04Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS04 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS04'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS04'){
...
module.exports = { handleSubscriptionPS04 }
