...
const { arregloTagsAntenasAllJosefina, arregloTagsInfoGeneralPSJosefina } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getInfoGeneralPSJosefina = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsInfoGeneralPSJosefina()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getAntenasAllJosefina = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsAntenasAllJosefina()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const executeAllFuntions = () => {
    getInfoGeneralPSJosefina(),
    getAntenasAllJosefina()
}

const handleSubscriptionInfoGeneralPSFJOS = (ws, message) => {
    if (message.toString() === 'subscribeFJOSInfoGeneralPS'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSInfoGeneralPS'){
...
module.exports = { handleSubscriptionInfoGeneralPSFJOS }
