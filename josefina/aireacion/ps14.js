...
const { generarTags, generarTagsTCP, arregloTagsPS14Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS14 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS14'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS14TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS14_TA_14_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS14 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS14'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS14'){
