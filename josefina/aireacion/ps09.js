...
const { generarTags, generarTagsTCP, arregloTagsPS09Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS09 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS09'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS09TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS09_TA_09_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS09 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS09'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS09'){
