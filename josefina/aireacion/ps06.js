...
const { generarTags, generarTagsTCP, arregloTagsPS06Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS06 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS06'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS06TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS06_TA_06_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS06 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS06'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS06'){
