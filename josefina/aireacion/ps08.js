...
const { generarTags, generarTagsTCP, arregloTagsPS08Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS08 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS08'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS08TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS08_TA_08_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS08 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS08'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS08'){
