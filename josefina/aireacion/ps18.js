...
const { generarTags, generarTagsTCP, arregloTagsPS18Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS18 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS18'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS18TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS18_TA_18_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS18 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS18'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS18'){
