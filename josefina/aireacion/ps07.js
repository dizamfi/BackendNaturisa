...
const { generarTags, generarTagsTCP, arregloTagsPS07Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS07 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS07'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS07TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS07_TA_07_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS07Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(18)] = ex.Value;     
                    });
                    dataCache.PS07Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS07 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS07'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS07'){
...
module.exports = { handleSubscriptionPS07 }
