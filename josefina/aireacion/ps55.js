...
const { generarTags, generarTagsTCP, arregloTagsPS55Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS55 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS55'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS55TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS55_TA_55_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS55 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS55'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS55'){
