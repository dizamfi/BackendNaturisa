...
const { generarTags, generarTagsTCP, arregloTagsPS02Antenas, arregloTagsPS03Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS03 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS03'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS03TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS03_TA_03_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS03 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS03'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS03'){
