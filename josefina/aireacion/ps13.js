...
const { generarTags, generarTagsTCP, arregloTagsPS13Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS13 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS13'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS13TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS13_TA_13_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS13 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS13'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS13'){
