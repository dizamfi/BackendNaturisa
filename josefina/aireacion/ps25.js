...
const { generarTags, generarTagsTCP, arregloTagsPS25Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS25 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS25'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS25TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS25_TA_25_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS25 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS25'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS25'){
