...
const { generarTags, generarTagsTCP, arregloTagsPC107Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPC107 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PC107'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC107TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PC107_TA_107_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC107 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPC107'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPC107'){
