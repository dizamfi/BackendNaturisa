...
const { generarTags, generarTagsTCP, arregloTagsPS24Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS24 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS24'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS24TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS24_TA_24_01','Josefina_FJOS_PS24_TA_24_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS24 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS24'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS24'){
