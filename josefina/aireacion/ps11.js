...
const { generarTags, generarTagsTCP, arregloTagsPS11Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS11 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS11'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS11TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS11_TA_11_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS11 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS11'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS11'){
