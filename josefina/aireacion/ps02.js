...
const { generarTags, generarTagsTCP, arregloTagsPS02Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS02 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS02TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS02_TA_02_01','Josefina_FJOS_PS02_TA_02_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS02 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS02'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS02'){
