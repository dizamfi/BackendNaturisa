...
const { generarTags, generarTagsTCP, arregloTagsPS50Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS50 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS50'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS50TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS50_TA_50_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS50 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS50'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS50'){
