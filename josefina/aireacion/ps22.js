...
const { generarTags, generarTagsTCP, arregloTagsPS22Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS22 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS22'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS22TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS22_TA_22_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS22 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS22'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS22'){
