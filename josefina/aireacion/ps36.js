...
const { generarTags, generarTagsTCP, arregloTagsPS36Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS36 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS36'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS36TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS36_TA_36_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS36 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS36'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS36'){
