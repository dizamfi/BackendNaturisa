...
const { generarTags, generarTagsTCP, arregloTagsPS01Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS01 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS01TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS01_TA_01_01','Josefina_FJOS_PS01_TA_01_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS01 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS01'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS01'){
