...
const { generarTags, generarTagsTCP, arregloTagsPC113Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPC113 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PC113'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC113TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PC113_TA_113_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC113 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPC113'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPC113'){
