...
const { generarTags, generarTagsTCP, arregloTagsPS32Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS32 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS32'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS32TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS32_TA_32_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS32 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS32'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS32'){
