...
const { generarTags, generarTagsTCP, arregloTagsPS45Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS45 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS45'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS45TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS45_TA_45_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS45 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS45'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS45'){
