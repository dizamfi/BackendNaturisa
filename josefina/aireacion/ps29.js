...
const { generarTags, generarTagsTCP, arregloTagsPS29Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS29 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS29'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS29TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS29_TA_29_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS29 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS29'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS29'){
