...
const { generarTags, generarTagsTCP, arregloTagsPS51Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS51 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS51'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS51TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS51_TA_51_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS51 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS51'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS51'){
