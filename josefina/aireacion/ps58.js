...
const { generarTags, generarTagsTCP, arregloTagsPS58Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS58 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS58'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS58TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS58_TA_58_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS58 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS58'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS58'){
