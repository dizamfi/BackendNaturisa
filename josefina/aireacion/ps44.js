...
const { generarTags, generarTagsTCP, arregloTagsPS44Antenas } = require('../../../helpers/josefina/arregloTagsAireacion');
...
const getPS44 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Josefina_FJOS_PS44'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS44TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Josefina_FJOS_PS44_TA_44_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS44 = (ws, message) => {
    if (message.toString() === 'subscribeFJOSPS44'){
    ...
    } else if (message.toString() === 'unsubscribeFJOSPS44'){
