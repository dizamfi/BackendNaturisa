...
const { arregloTagsPC111Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPC111 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PC111'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC111TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PC111_TA_111_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC111 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPC111'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPC111'){
