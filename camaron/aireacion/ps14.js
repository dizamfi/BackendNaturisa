...
const { arregloTagsPS14Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPS14 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PS14'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS14TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PS14_TA_14_01','Camaron_FCAM_PS14_TA_14_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS14 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPS14'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPS14'){
