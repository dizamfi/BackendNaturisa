...
const { arregloTagsPS18Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPS18 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PS18'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS18TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PS18_TA_18_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS18 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPS18'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPS18'){
