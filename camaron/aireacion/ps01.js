...
const { arregloTagsPS01Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPS01 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PS01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS01TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PS01_TA_01_01','Camaron_FCAM_PS01_TA_01_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS01 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPS01'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPS01'){
