...
const { arregloTagsPS10Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPS10 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PS10'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS10TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PS10_TA_10_01','Camaron_FCAM_PS10_TA_10_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS10 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPS10'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPS10'){
