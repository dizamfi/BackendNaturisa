...
const { arregloTagsPC107Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPC107 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PC107'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC107TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PC107_TA_107_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC107 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPC107'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPC107'){
