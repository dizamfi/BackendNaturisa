...
const { arregloTagsAntenasAllCamaron } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getAntenasAllCamaron = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsAntenasAllCamaron()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    ...
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(16)] = ex.Value;     
                    });
...
const executeAllFuntions = () => {
    getAntenasAllCamaron()
}

const handleSubscriptionAntenasAllCamaron = (ws, message) => {
    if (message.toString() === 'subscribeFCAMAntenas'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMAntenas'){
...
module.exports = { handleSubscriptionAntenasAllCamaron }
