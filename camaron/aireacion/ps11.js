...
const { arregloTagsPS11Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPS11 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PS11'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS11TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PS11_TA_11_01','Camaron_FCAM_PS11_TA_11_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS11 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPS11'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPS11'){
