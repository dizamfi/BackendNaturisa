...
const { arregloTagsPC113Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPC113 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PC113'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC113TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PC113_TA_113_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC113 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPC113'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPC113'){
