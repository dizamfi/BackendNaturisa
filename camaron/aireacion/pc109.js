...
const { arregloTagsPC109Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPC109 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PC109'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC109TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PC109_TA_109_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC109 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPC109'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPC109'){
