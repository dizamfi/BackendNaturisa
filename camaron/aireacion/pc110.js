...
const { arregloTagsPC110Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPC110 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PC110'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC110TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PC110_TA_110_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC110 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPC110'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPC110'){
