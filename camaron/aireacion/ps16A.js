...
const { arregloTagsPS16AAntenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPS16A = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PS16A'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS16ATCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PS16A_TA_16A_01','Camaron_FCAM_PS16A_TA_16A_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS16A = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPS16A'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPS16A'){
