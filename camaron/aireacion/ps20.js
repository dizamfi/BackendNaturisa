...
const { arregloTagsPS20Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPS20 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PS20'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS20TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PS20_TA_20_01','Camaron_FCAM_PS20_TA_20_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS20 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPS20'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPS20'){
