...
const { arregloTagsPS12Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPS12 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PS12'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS12TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PS12_TA_12_01','Camaron_FCAM_PS12_TA_12_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS12 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPS12'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPS12'){
