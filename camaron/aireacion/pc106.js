...
const { arregloTagsPC106Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPC106 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PC106'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC106TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PC106_TA_106_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC106 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPC106'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPC106'){
