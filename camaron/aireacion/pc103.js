...
const { arregloTagsPC103Antenas, generarTags, generarTagsTA } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPC103 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PC103'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC103TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTA(['Camaron_FCAM_PC103_TA_103_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC103 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPC103'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPC103'){
