...
const { arregloTagsPC112Antenas, generarTags, generarTagsTCP } = require('../../../helpers/camaron/arregloTagsAireacion');
...
const getPC112 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Camaron_FCAM_PC112'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC112TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Camaron_FCAM_PC112_TA_112_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC112 = (ws, message) => {
    if (message.toString() === 'subscribeFCAMPC112'){
    ...
    } else if (message.toString() === 'unsubscribeFCAMPC112'){
