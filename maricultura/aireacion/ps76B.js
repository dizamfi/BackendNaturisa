...
const { generarTags, generarTagsTCP, arregloTagsPS76BAntenas } = require('../../../helpers/maricultura/arregloTagsAireacion');
...
const getPS76B = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Maricultura_FMARC_PS76B'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS76BTCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Maricultura_FMARC_PS76B_TA_76B_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPS76BAntenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPS76BAntenas()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS76BAntenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(4)] = ex.Value;     
                    });
                    dataCache.PS76BAntenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS76B = (ws, message) => {
    if (message.toString() === 'subscribeFMARCPS76B'){
    ...
    } else if (message.toString() === 'unsubscribeFMARCPS76B'){
...
module.exports = { handleSubscriptionPS76B }
