...
const { generarTags, generarTagsTCP, arregloTagsPS53Antenas } = require('../../../helpers/maricultura/arregloTagsAireacion');
...
const getPS53 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Maricultura_FMARC_PS53'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS53TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Maricultura_FMARC_PS53_TA_53_01','Maricultura_FMARC_PS53_TA_53_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS53 = (ws, message) => {
    if (message.toString() === 'subscribeFMARCPS53'){
    ...
    } else if (message.toString() === 'unsubscribeFMARCPS53'){
