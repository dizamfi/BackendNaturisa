...
const { generarTags, generarTagsTCP, arregloTagsPS58Antenas } = require('../../../helpers/maricultura/arregloTagsAireacion');
...
const getPS58 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Maricultura_FMARC_PS58'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS58TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Maricultura_FMARC_PS58_TA_58_01','Maricultura_FMARC_PS58_TA_58_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPS58Antenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPS58Antenas()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS58Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(4)] = ex.Value;     
                    });
                    dataCache.PS58Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS58 = (ws, message) => {
    if (message.toString() === 'subscribeFMARCPS58'){
    ...
    } else if (message.toString() === 'unsubscribeFMARCPS58'){
...
module.exports = { handleSubscriptionPS58 }
