...
const { generarTags, generarTagsTCP, arregloTagsPC107AAntenas } = require('../../../helpers/maricultura/arregloTagsAireacion');
...
const getPC107A = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Maricultura_FMARC_PC107A'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    ...
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(25)] = ex.Value;     
                    });
                    dataCache.PC107A.push(obje)
                    obje = {}
                    completedFuntions++
...
const getPC107ATCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Maricultura_FMARC_PC107A_TA_107A_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPC107AAntenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPC107AAntenas()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PC107AAntenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(4)] = ex.Value;     
                    });
                    dataCache.PC107AAntenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPC107A = (ws, message) => {
    if (message.toString() === 'subscribeFMARCPC107A'){
    ...
    } else if (message.toString() === 'unsubscribeFMARCPC107A'){
...
module.exports = { handleSubscriptionPC107A }
