...
const { generarTags, generarTagsTCP, arregloTagsPS81Antenas } = require('../../../helpers/maricultura/arregloTagsAireacion');
...
const getPS81 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Maricultura_FMARC_PS81'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS81TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Maricultura_FMARC_PS81_TA_81_01','Maricultura_FMARC_PS81_TA_81_02','Maricultura_FMARC_PS81_TA_81_03'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPS81Antenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPS81Antenas()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS81Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(4)] = ex.Value;     
                    });
                    dataCache.PS81Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS81 = (ws, message) => {
    if (message.toString() === 'subscribeFMARCPS81'){
    ...
    } else if (message.toString() === 'unsubscribeFMARCPS81'){
...
module.exports = { handleSubscriptionPS81 }
