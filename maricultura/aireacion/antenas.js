...
const { arregloTagsAntenasAllMaricultura } = require('../../../helpers/maricultura/arregloTagsAireacion');
...
const getAntenasAllMaricultura = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsAntenasAllMaricultura()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const executeAllFuntions = () => {
    getAntenasAllMaricultura()
}

const handleSubscriptionAntenasAllMaricultura = (ws, message) => {
    if (message.toString() === 'subscribeFMARCAntenas'){
    ...
    } else if (message.toString() === 'unsubscribeFMARCAntenas'){
...
module.exports = { handleSubscriptionAntenasAllMaricultura }
