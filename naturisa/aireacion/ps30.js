...
const { generarTags, generarTagsTCP, arregloTagsPS30Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS30 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS30'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS30TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS30_TA_30_01','Naturisa_FNTS_PS30_TA_30_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS30 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS30'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS30'){
