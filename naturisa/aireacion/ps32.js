...
const { generarTags, generarTagsTCP, arregloTagsPS32Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS32 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS32'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS32TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS32_TA_32_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS32 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS32'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS32'){
