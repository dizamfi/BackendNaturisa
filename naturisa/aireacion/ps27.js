...
const { generarTags, generarTagsTCP, arregloTagsPS27Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS27 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS27'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS27TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS27_TA_27_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS27 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS27'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS27'){
