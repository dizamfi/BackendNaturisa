...
const { generarTags, generarTagsTCP, arregloTagsPS03Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS03 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS03'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS03TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS03_TA_03_01','Naturisa_FNTS_PS03_TA_03_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS03 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS03'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS03'){
...
/* wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        ...
        if (message.toString() === 'subscribePS03'){
            console.log('Cliente suscrito')
            console.log('aquii')
            clientSubscriptions.set(ws, true);
        ...
        } else if (message.toString() === 'unsubscribePS03'){
            clientSubscriptions.set(ws, false);
            console.log('Cliente desuscrito')
        }
