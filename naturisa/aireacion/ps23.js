...
const { generarTags, generarTagsTCP, arregloTagsPS23Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS23 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS23'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS23TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS23_TA_23_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS23 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS23'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS23'){
