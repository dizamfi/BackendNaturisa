...
const { generarTags, generarTagsTCP, arregloTagsPS21Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS21 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS21'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS21TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS21_TA_21_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS21 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS21'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS21'){
