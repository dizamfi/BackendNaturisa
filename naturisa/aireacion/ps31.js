...
const { generarTags, generarTagsTCP, arregloTagsPS31Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS31 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS31'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS31TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS31_TA_31_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS31 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS31'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS31'){
