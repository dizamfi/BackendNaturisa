...
const { generarTags, generarTagsTCP, arregloTagsPS10Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS10 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS10'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS10TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS10_TA_10_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS10 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS10'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS10'){
