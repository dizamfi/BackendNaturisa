...
const { generarTags, generarTagsTCP, arregloTagsPS18Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS18 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS18'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS18TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS18_TA_18_01','Naturisa_FNTS_PS18_TA_18_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS18Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(18)] = ex.Value;     
                    });
                    dataCache.PS18Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS18 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS18'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS18'){
...
module.exports = { handleSubscriptionPS18 }
