...
const { generarTags, generarTagsTCP, arregloTagsPS19Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS19 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS19'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS19TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS19_TA_19_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS19 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS19'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS19'){
