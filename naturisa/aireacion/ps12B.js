...
const { generarTags, generarTagsTCP, arregloTagsPS12BAntenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS12B = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS12B'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS12BTCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS12B_TA_12B_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS12B = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS12B'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS12B'){
