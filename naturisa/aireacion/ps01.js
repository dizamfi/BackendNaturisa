...
const { generarTags, generarTagsTCP, arregloTagsPS01Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS01 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS01TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS01_TA_01_01','Naturisa_FNTS_PS01_TA_01_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS01 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS01'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS01'){
