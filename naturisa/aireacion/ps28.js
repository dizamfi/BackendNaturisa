...
const { generarTags, generarTagsTCP, arregloTagsPS28Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS28 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS28'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS28TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS28_TA_28_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS28 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS28'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS28'){
