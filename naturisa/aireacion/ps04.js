...
const { generarTags, generarTagsTCP, arregloTagsPS04Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS04 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS04'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS04TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS04_TA_04_01','Naturisa_FNTS_PS04_TA_04_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS04 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS04'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS04'){
...
/* wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        ...
        if (message.toString() === 'subscribePS04'){
            console.log('Cliente suscrito')
            console.log('aquii')
            clientSubscriptions.set(ws, true);
        ...
        } else if (message.toString() === 'unsubscribePS04'){
            clientSubscriptions.set(ws, false);
            console.log('Cliente desuscrito')
        }
