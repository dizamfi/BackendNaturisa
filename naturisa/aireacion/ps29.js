...
const { generarTags, generarTagsTCP, arregloTagsPS29Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS29 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS29'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS29TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS29_TA_29_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS29 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS29'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS29'){
