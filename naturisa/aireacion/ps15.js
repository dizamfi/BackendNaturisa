...
const { generarTags, generarTagsTCP, arregloTagsPS15Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS15 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS15'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS15TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS15_TA_15_01','Naturisa_FNTS_PS15_TA_15_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS15Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(18)] = ex.Value;     
                    });
                    dataCache.PS15Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS15 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS15'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS15'){
...
module.exports = { handleSubscriptionPS15 }
