...
const { generarTags, generarTagsTCP, arregloTagsPS02Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS02 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS02TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS02_TA_02_01','Naturisa_FNTS_PS02_TA_02_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS02 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS02'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS02'){
...
/* wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        ...
        if (message.toString() === 'subscribePS02'){
            console.log('Cliente suscrito')
            console.log('aquii')
            clientSubscriptions.set(ws, true);
        ...
        } else if (message.toString() === 'unsubscribePS02'){
            clientSubscriptions.set(ws, false);
            console.log('Cliente desuscrito')
        }
