...
const { generarTags, generarTagsTCP, arregloTagsPS20Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS20 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS20'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS20TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS20_TA_20_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS20 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS20'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS20'){
