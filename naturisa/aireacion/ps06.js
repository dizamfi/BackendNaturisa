...
const { generarTags, generarTagsTCP, arregloTagsPS06Antenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS06 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS06'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS06TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS06_TA_06_01','Naturisa_FNTS_PS06_TA_06_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS06 = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS06'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS06'){
...
/* wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        ...
        if (message.toString() === 'subscribePS06'){
            console.log('Cliente suscrito')
            console.log('aquii')
            clientSubscriptions.set(ws, true);
        ...
        } else if (message.toString() === 'unsubscribePS06'){
            clientSubscriptions.set(ws, false);
            console.log('Cliente desuscrito')
        }
