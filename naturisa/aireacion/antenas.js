...
const { arregloTagsAntenasAllNaturisa } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getAntenasAllNaturisa = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsAntenasAllNaturisa()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const executeAllFuntions = () => {
    getAntenasAllNaturisa()
}

const handleSubscriptionAntenasAllNaturisa = (ws, message) => {
    if (message.toString() === 'subscribeFNTSAntenas'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSAntenas'){
...
module.exports = { handleSubscriptionAntenasAllNaturisa }
