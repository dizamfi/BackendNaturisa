...
const { generarTags, generarTagsTCP, arregloTagsPS16BAntenas } = require('../../../helpers/naturisa/arregloTagsAireacion');
...
const getPS16B = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Naturisa_FNTS_PS16B'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS16BTCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Naturisa_FNTS_PS16B_TA_16B_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS16B = (ws, message) => {
    if (message.toString() === 'subscribeFNTSPS16B'){
    ...
    } else if (message.toString() === 'unsubscribeFNTSPS16B'){
