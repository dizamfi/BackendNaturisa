...
const { generarTags, generarTagsTCP, arregloTagsPS43Antenas } = require('../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS43 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['RioNilo_FRNL_PS43'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS43TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['RioNilo_FRNL_PS43_TA_43_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS43 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLPS43'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLPS43'){
