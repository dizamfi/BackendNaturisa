...
const { generarTags, generarTagsTCP, arregloTagsPS29Antenas } = require('../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS29 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['RioNilo_FRNL_PS29'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS29TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['RioNilo_FRNL_PS29_TA_29_01','RioNilo_FRNL_PS29_TA_29_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS29 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLPS29'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLPS29'){
