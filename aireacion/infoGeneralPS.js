...
const { arregloTagsAntenasAllRioNilo, arregloTagsInfoGeneralPSRioNilo } = require('../../../helpers/rioNilo/arregloTagsAireacion');
...
const getInfoGeneralPSRioNilo = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsInfoGeneralPSRioNilo()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getAntenasAllRioNilo = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsAntenasAllRioNilo()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const executeAllFuntions = () => {
    getInfoGeneralPSRioNilo(),
    getAntenasAllRioNilo()
}

const handleSubscriptionInfoGeneralPSFRNL = (ws, message) => {
    if (message.toString() === 'subscribeFRNLInfoGeneralPS'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLInfoGeneralPS'){
...
module.exports = { handleSubscriptionInfoGeneralPSFRNL }
