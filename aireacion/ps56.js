...
const { generarTags, generarTagsTCP, arregloTagsPS56Antenas } = require('../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS56 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['RioNilo_FRNL_PS56'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS56TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['RioNilo_FRNL_PS56_TA_56_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS56 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLPS56'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLPS56'){
