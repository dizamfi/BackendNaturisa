...
const { generarTags, generarTagsTCP, arregloTagsPS55Antenas } = require('../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS55 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['RioNilo_FRNL_PS55'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS55TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['RioNilo_FRNL_PS55_TA_55_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS55 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLPS55'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLPS55'){
