...
const { generarTags, generarTagsTCP, arregloTagsPS36Antenas } = require('../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS36 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['RioNilo_FRNL_PS36'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS36TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['RioNilo_FRNL_PS36_TA_36_01','RioNilo_FRNL_PS36_TA_36_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS36 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLPS36'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLPS36'){
