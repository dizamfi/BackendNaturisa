...
const { generarTags, generarTagsTCP, arregloTagsPC111Antenas } = require('../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPC111 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['RioNilo_FRNL_PC111'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC111TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['RioNilo_FRNL_PC111_TA_111_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC111 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLPC111'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLPC111'){
