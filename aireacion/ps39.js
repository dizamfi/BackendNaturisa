...
const { generarTags, generarTagsTCP, arregloTagsPS39Antenas } = require('../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS39 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['RioNilo_FRNL_PS39'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS39TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['RioNilo_FRNL_PS39_TA_39_01','RioNilo_FRNL_PS39_TA_39_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS39 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLPS39'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLPS39'){
