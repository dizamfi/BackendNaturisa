...
const { generarTags, generarTagsTCP, arregloTagsPC110Antenas } = require('../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPC110 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['RioNilo_FRNL_PC110'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC110TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['RioNilo_FRNL_PC110_TA_110_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPC110Antenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPC110Antenas()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PC110Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(4)] = ex.Value;     
                    });
                    dataCache.PC110Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPC110 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLPC110'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLPC110'){
...
module.exports = { handleSubscriptionPC110 }
