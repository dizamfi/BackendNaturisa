...
const { arregloTagsPS15Antenas, generarTagsSur, generarTagsTCPSur } = require('../../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS15 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsSur(['Rnl_Sur_PS15'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS15TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCPSur(['Rnl_Sur_PS15_TA01','Rnl_Sur_PS15_TA02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS15 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLSPS15'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLSPS15'){
