...
const { arregloTagsPS10Antenas, generarTagsSur, generarTagsTCPSur } = require('../../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS10 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsSur(['Rnl_Sur_PS10'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS10TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCPSur(['Rnl_Sur_PS10_TA01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS10 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLSPS10'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLSPS10'){
