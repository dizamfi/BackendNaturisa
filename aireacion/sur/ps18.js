...
const { arregloTagsPS18Antenas, generarTagsSur, generarTagsTCPSur } = require('../../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS18 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsSur(['Rnl_Sur_PS18'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS18TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCPSur(['Rnl_Sur_PS18_TA01','Rnl_Sur_PS18_TA02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS18 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLSPS18'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLSPS18'){
