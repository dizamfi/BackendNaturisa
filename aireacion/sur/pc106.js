...
const { arregloTagsPC106Antenas, generarTagsSur, generarTagsTCPSur } = require('../../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPC106 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsSur(['Rnl_Sur_PC106'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPC106TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCPSur(['Rnl_Sur_PC106_TA01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPC106 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLSPC106'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLSPC106'){
