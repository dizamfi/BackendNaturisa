...
const { arregloTagsPS01Antenas, generarTagsSur, generarTagsTCPSur } = require('../../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS01 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsSur(['Rnl_Sur_PS01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS01TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCPSur(['Rnl_Sur_PS01_TA01','Rnl_Sur_PS01_TA02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS01 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLSPS01'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLSPS01'){
