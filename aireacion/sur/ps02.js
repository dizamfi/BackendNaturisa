...
const { arregloTagsPS02Antenas, generarTagsSur, generarTagsTCPSur } = require('../../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS02 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsSur(['Rnl_Sur_PS02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS02TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCPSur(['Rnl_Sur_PS02_TA01','Rnl_Sur_PS02_TA02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS02 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLSPS02'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLSPS02'){
