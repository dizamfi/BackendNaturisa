...
const { arregloTagsPS14Antenas, generarTagsSur, generarTagsTCPSur } = require('../../../../helpers/rioNilo/arregloTagsAireacion');
...
const getPS14 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsSur(['Rnl_Sur_PS14'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS14TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCPSur(['Rnl_Sur_PS14_TA01','Rnl_Sur_PS14_TA02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS14 = (ws, message) => {
    if (message.toString() === 'subscribeFRNLSPS14'){
    ...
    } else if (message.toString() === 'unsubscribeFRNLSPS14'){
