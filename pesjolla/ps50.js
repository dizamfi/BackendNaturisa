...
const { generarTags, generarTagsTCP, arregloTagsPS50Antenas } = require('../../helpers/pesjolla/arregloTagsAireacion');
...
const getPS50 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Pesjolla_FPSJ_PS50'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS50TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Pesjolla_FPSJ_PS50_TA_50_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS50 = (ws, message) => {
    if (message.toString() === 'subscribeFPSJPS50'){
    ...
    } else if (message.toString() === 'unsubscribeFPSJPS50'){
