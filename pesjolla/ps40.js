...
const { generarTags, generarTagsTCP, arregloTagsPS40Antenas } = require('../../helpers/pesjolla/arregloTagsAireacion');
...
const getPS40 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Pesjolla_FPSJ_PS40'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS40TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Pesjolla_FPSJ_PS40_TA_40_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS40 = (ws, message) => {
    if (message.toString() === 'subscribeFPSJPS40'){
    ...
    } else if (message.toString() === 'unsubscribeFPSJPS40'){
