...
const { generarTags, generarTagsTCP, arregloTagsPS33Antenas } = require('../../helpers/pesjolla/arregloTagsAireacion');
...
const getPS33 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Pesjolla_FPSJ_PS33'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS33TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Pesjolla_FPSJ_PS33_TA_33_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS33 = (ws, message) => {
    if (message.toString() === 'subscribeFPSJPS33'){
    ...
    } else if (message.toString() === 'unsubscribeFPSJPS33'){
