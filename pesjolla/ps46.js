...
const { generarTags, generarTagsTCP, arregloTagsPS46Antenas } = require('../../helpers/pesjolla/arregloTagsAireacion');
...
const getPS46 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Pesjolla_FPSJ_PS46'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS46TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Pesjolla_FPSJ_PS46_TA_46_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS46 = (ws, message) => {
    if (message.toString() === 'subscribeFPSJPS46'){
    ...
    } else if (message.toString() === 'unsubscribeFPSJPS46'){
