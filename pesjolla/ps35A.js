...
const { generarTags, generarTagsTCP, arregloTagsPS35AAntenas } = require('../../helpers/pesjolla/arregloTagsAireacion');
...
const getPS35A = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Pesjolla_FPSJ_PS35A'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS35ATCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Pesjolla_FPSJ_PS35A_TA_35A_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const getPS35AAntenas = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${arregloTagsPS35AAntenas()}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PS35AAntenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(18)] = ex.Value;     
                    });
                    dataCache.PS35AAntenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPS35A = (ws, message) => {
    if (message.toString() === 'subscribeFPSJPS35A'){
    ...
    } else if (message.toString() === 'unsubscribeFPSJPS35A'){
...
module.exports = { handleSubscriptionPS35A }
