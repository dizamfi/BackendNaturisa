...
const { generarTags, generarTagsTCP, arregloTagsPS38Antenas } = require('../../helpers/pesjolla/arregloTagsAireacion');
...
const getPS38 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Pesjolla_FPSJ_PS38'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS38TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Pesjolla_FPSJ_PS38_TA_38_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS38 = (ws, message) => {
    if (message.toString() === 'subscribeFPSJPS38'){
    ...
    } else if (message.toString() === 'unsubscribeFPSJPS38'){
