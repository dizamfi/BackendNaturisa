...
const { generarTags, generarTagsTCP, arregloTagsPS45Antenas } = require('../../helpers/pesjolla/arregloTagsAireacion');
...
const getPS45 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Pesjolla_FPSJ_PS45'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS45TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Pesjolla_FPSJ_PS45_TA_45_01','Pesjolla_FPSJ_PS45_TA_45_02'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS45 = (ws, message) => {
    if (message.toString() === 'subscribeFPSJPS45'){
    ...
    } else if (message.toString() === 'unsubscribeFPSJPS45'){
