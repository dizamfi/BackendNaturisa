...
const { generarTags, generarTagsTCP, arregloTagsPS44Antenas } = require('../../helpers/pesjolla/arregloTagsAireacion');
...
const getPS44 = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTags(['Pesjolla_FPSJ_PS44'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
...
const getPS44TCP = () => {
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            let tagReadCommand = `{"Message":"ReadTag","Params":{"Tags": ${generarTagsTCP(['Pesjolla_FPSJ_PS44_TA_44_01'])}},"ClientCookie":"myReadTagRequest1"}\n`;
            let obje = {};
            client.write(tagReadCommand);
...
const handleSubscriptionPS44 = (ws, message) => {
    if (message.toString() === 'subscribeFPSJPS44'){
    ...
    } else if (message.toString() === 'unsubscribeFPSJPS44'){
